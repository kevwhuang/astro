...
test('text', async ({ context, page }) => {
    ...
    await expect(newPage.locator('css=pre')).toHaveCount(1);
});
...
test('html', async ({ context, page }) => {
    ...
    await expect(newPage.locator('css=main')).toHaveCount(1);
});
...
test('audio', async ({ browserName, context, page }) => {
    ...
    if (browserName === 'chromium') {
        const newPage = await context.waitForEvent('page');
        await expect(newPage.locator('css=video')).toHaveCount(1);
    }
});
